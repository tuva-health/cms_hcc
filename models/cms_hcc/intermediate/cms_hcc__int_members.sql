...
, payment_year_age_dates as (
    ...
    select distinct
          payment_year
        , cast({{ the_tuva_project.concat_custom(['payment_year',"'-02-01'"]) }} as date) as payment_year_age_date
...
, cap_collection_start_end_dates as (
    ...
    select
        ...
        , collection_end_date
        , case
            when enrollment_start_date < {{ the_tuva_project.try_to_cast_date('collection_start_date', 'YYYY-MM-DD') }}
            then {{ the_tuva_project.try_to_cast_date('collection_start_date', 'YYYY-MM-DD') }}
