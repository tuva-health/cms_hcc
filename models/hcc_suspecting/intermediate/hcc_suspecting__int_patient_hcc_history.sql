...
, add_flag as (
    ...
    select
        ...
        , case
            when {{ the_tuva_project.date_part('year', 'hcc_billed.last_billed') }} = {{ the_tuva_project.date_part('year', dbt.current_timestamp()) }}
            then 1
            else 0
...
, all_conditions_with_flag as (
    ...
    select distinct
        ...
        , {{ the_tuva_project.concat_custom([
            ...
            the_tuva_project.concat_custom(["' last billed on '", "last_billed"]) ~
            " when last_billed is null and last_recorded is not null then " ~
            the_tuva_project.concat_custom(["' last recorded on '", "last_recorded"]) ~
